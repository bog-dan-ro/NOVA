...
template<Ec::cont_t C> void Ec::send_msg (Ec *const self)
{
...
void Ec::sys_ipc_call (Ec *const self)
{
    ...
    if (!cpt.validate (Capability::Perm_pt::CALL)) [[unlikely]]
...
void Ec::sys_create_pd (Ec *const self)
{
    ...
    if (!cpd.validate (Capability::Perm_pd::PD)) [[unlikely]]
...
void Ec::sys_create_ec (Ec *const self)
{
    ...
    if (!cpd.validate (Capability::Perm_pd::EC)) [[unlikely]]
        self->sys_finish_status (Status::BAD_CAP);
    ...
    Status s;
    pd->create_ec (s, obj, r.sel(), r.cpu(), r.evt(), r.sp(), r.hva(), r.flg());
...
void Ec::sys_create_sc (Ec *const self)
{
    ...
    if (!r.prio() || !r.budget() || !Cos::valid_cos (r.cos())) [[unlikely]]
        self->sys_finish_status (Status::BAD_PAR);
    ...
    if (!cpd.validate (Capability::Perm_pd::SC) || !cec.validate (Capability::Perm_ec::BIND_SC)) [[unlikely]]
        self->sys_finish_status (Status::BAD_CAP);

    // Determine owner PD and bound EC
    auto const pd { static_cast<Pd *>(cpd.obj()) };
    auto const ec { static_cast<Ec *>(cec.obj()) };

    // EC and SC must belong to the same PD
    if (ec->get_pd() != pd || ec->subtype == Kobject::Subtype::EC_LOCAL) [[unlikely]]
        self->sys_finish_status (Status::BAD_CAP);
...
void Ec::sys_create_pt (Ec *const self)
{
    ...
    // EC and PT must belong to the same PD
    if (ec->get_pd() != pd || ec->subtype != Kobject::Subtype::EC_LOCAL) [[unlikely]]
        self->sys_finish_status (Status::BAD_CAP);
...
void Ec::sys_create_sm (Ec *const self)
{
    ...
    bool const i { !!(r.flg() & BIT (0)) };

    trace (TRACE_SYSCALL, "EC:%p %s SEL:%#lx PD:%#lx %s:%#lx", static_cast<void *>(self), __func__, r.sel(), r.pd(), i ? "GSI" : "CNT", r.val());

    auto const obj { self->regs.get_obj() };
    auto const cpd { obj->lookup (r.pd()) };

    if (!cpd.validate (i ? Capability::Perm_pd::DC : Capability::Perm_pd::SM)) [[unlikely]]
        self->sys_finish_status (Status::BAD_CAP);
    ...
    void *ptr { nullptr };
    if (i && !(ptr = Interrupt::get_ptr (static_cast<iid_t>(r.val())))) [[unlikely]]
        self->sys_finish_status (Status::BAD_PAR);

    Status s;
    pd->create_sm (s, obj, r.sel(), r.val(), ptr);
...
void Ec::sys_ctrl_pd (Ec *const self)
{
    ...
    if ((r.ssb() | r.dsb()) & (BITN (r.ord()) - 1)) [[unlikely]]
        self->sys_finish_status (Status::BAD_PAR);
    ...
    if (Capability::validate_take_grant (cst, cdt, st, dt)) [[likely]] {
...
void Ec::sys_ctrl_ec (Ec *const self)
{
    ...
    if (!cec.validate (Capability::Perm_ec::CTRL)) [[unlikely]]
        self->sys_finish_status (Status::BAD_CAP);
...
void Ec::sys_ctrl_sc (Ec *const self)
{
    ...
    if (!csc.validate (Capability::Perm_sc::CTRL)) [[unlikely]]
        self->sys_finish_status (Status::BAD_CAP);
...
void Ec::sys_ctrl_sm (Ec *const self)
{
    ...
    if (!csm.validate (r.op() ? Capability::Perm_sm::CTRL_DN : Capability::Perm_sm::CTRL_UP)) [[unlikely]]
        self->sys_finish_status (Status::BAD_CAP);
    ...
    if (r.op()) {           // Down
        ...
        if (sm->subtype == Kobject::Subtype::SM_INT)
            Interrupt::deactivate (sm);

        s = sm->dn (self, r.zc(), r.time_ticks());
...
void Ec::sys_ctrl_hw (Ec *const self)
{
    ...
    switch (r.op()) {
        ...
        case 7:             // MBA L2 Delay
            self->sys_finish_status (Cos::cfg_mb_thrt (static_cast<uint16_t>(r.desc()), static_cast<uint16_t>(r.desc() >> 16)));

        case 6:             // CAT L2 Capacity Bitmask
            self->sys_finish_status (Cos::cfg_l2_mask (static_cast<uint16_t>(r.desc()), static_cast<uint32_t>(r.desc() >> 16)));

        case 5:             // CAT L3 Capacity Bitmask
            self->sys_finish_status (Cos::cfg_l3_mask (static_cast<uint16_t>(r.desc()), static_cast<uint32_t>(r.desc() >> 16)));
...
void Ec::sys_assign_int (Ec *const self)
{
    ...
    // Check SM capability
    auto const csm { obj->lookup (r.sm()) };
    if (!csm.validate (Capability::Perm_sm::ASSIGN, Kobject::Subtype::SM_INT)) [[unlikely]]
        self->sys_finish_status (Status::BAD_CAP);
...
template<Status S, bool T> void Ec::sys_finish (Ec *const self)
{
    if (T)
        self->clr_timeout();
