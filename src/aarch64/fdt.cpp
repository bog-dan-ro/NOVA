...
void Fdt::Header::parse_subtree (Unaligned_be<uint32_t> const *&w, unsigned pa_cells, unsigned ps_cells, unsigned l) const
{
    unsigned const indent { 4 };
    unsigned a_cells { -1U }, s_cells { -1U };

    uint32_t l_interrupts { 0 }; Unaligned_be<uint32_t> const *p_interrupts { nullptr };
    uint32_t l_ranges     { 0 }; Unaligned_be<uint32_t> const *p_ranges     { nullptr };
    uint32_t l_reg        { 0 }; Unaligned_be<uint32_t> const *p_reg        { nullptr };
            ...
            case FDT_BEGIN_NODE:
                {
                    ...
                    while (*p++) ;
                    w = reinterpret_cast<Unaligned_be<uint32_t> const *>((reinterpret_cast<uintptr_t>(p) + 3) & ~3);
            ...
            case FDT_PROP:
                {
                    ...
                    w = reinterpret_cast<Unaligned_be<uint32_t> const *>((reinterpret_cast<uintptr_t>(w + 3) + len + 3) & ~3);
                    continue;
                }
