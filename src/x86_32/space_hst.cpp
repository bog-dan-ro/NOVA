...
Space_hst::Space_hst() : Space_mem { Kobject::Subtype::HST }
{
    ...
    // FIXME: Create an L1 PTAB for early sharing before CPUs plug themselves into the array.
    (void) Hptp::master.walk (MMAP_GLB_CPUS, 1, true);

    nova.hptp = Hptp::master;

    // 32-bit address space: max 4 GiB
    uintptr_t max_addr { BIT (Hpt::ibits - 1) };

    // Compute image addresses within mappable PA bounds
    auto const s { min (max_addr, static_cast<uintptr_t>(Kmem::sym_to_phys (&NOVA_HPAS))) };
    auto const e { min (max_addr, static_cast<uintptr_t>(Multiboot::ea)) };
