...
template<typename T, bool B> class Unaligned
{
    ...
    public:
        ...
        constexpr Unaligned (T v) { Aligned<T, B> tmp { v }; __builtin_memcpy (val, &tmp, sizeof (T)); }

        constexpr operator T() const { Aligned<T, B> tmp; __builtin_memcpy (&tmp, val, sizeof (T)); return tmp; }
};
