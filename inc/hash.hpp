...
class Sha
{
    private:
        ...
        // Section 4.2: The first 64 bits of the fractional parts of the cube roots of the first 80 prime numbers
    ...
    protected:
        // Section 5.1, 5.2
        template<typename T> void preprocess (T *h, uint8_t const *m, size_t s, bool more, void (*hash)(T *, uint8_t const *))
        {
        ...
        // Section 6.1
        template<typename T> static void sha1 (T *h, uint8_t const *m)
        {
            T w[80];

            // Prepare message schedule
            for (unsigned i { 0 }; i < 16; i++)
                w[i] = reinterpret_cast<Unaligned_be<T> const *>(m)[i];

            for (unsigned i { 16 }; i < sizeof (w) / sizeof (*w); i++)
                w[i] = rol (w[i - 3] ^ w[i - 8] ^ w[i - 14] ^ w[i - 16], 1);

            // Initialize working variables with previous hash value
            T v[5] { h[0], h[1], h[2], h[3], h[4] };

            // Compression
            for (unsigned i { 0 }; i < sizeof (w) / sizeof (*w); i++) {

                T t { w[i] + rol (v[0], 5) + v[4] };
        ...
        template<typename T> static void sha2 (T *h, uint8_t const *m)
        {
            T w[4 * sizeof (T) + 48];
            ...
            // Initialize working variables with previous hash value
            T v[8] { h[0], h[1], h[2], h[3], h[4], h[5], h[6], h[7] };

            // Compression
            for (unsigned i { 0 }; i < sizeof (w) / sizeof (*w); i++) {

                T const t1 { w[i] + v[7] + sum1 (v[4]) + chx (v[4], v[5], v[6]) + static_cast<T>(cbr[i] >> (64 - 8 * sizeof (T))) };
                T const t2 { sum0 (v[0]) + maj (v[0], v[1], v[2]) };
...
template<uint16_t H, unsigned D, typename T, T... I> class Hash : private Sha
{
    private:
        T h[sizeof...(I)] { I... };

    public:
        static constexpr auto blk_size { 16 * sizeof (T) };     // Block Size
        static constexpr auto dig_size { D / 8 };               // Digest Size
        static constexpr auto tpm_hash { H };                   // TPM Hash Algorithm ID

        void digest (uint8_t *p) const
        {
            for (unsigned i { 0 }; i < dig_size / sizeof (T); i++)
                reinterpret_cast<Unaligned_be<T> *>(p)[i] = h[i];
        }

        auto update (uint8_t const *m, size_t s, bool more = false) { preprocess (h, m, s, more, D == 160 ? sha1<T> : sha2<T>); return *this; }
};

// Section 5.3
using Hash_sha1_160 = Hash<0x4, 160, uint32_t, 0x67452301, 0xefcdab89, 0x98badcfe, 0x10325476, 0xc3d2e1f0>;
using Hash_sha2_224 = Hash<0x0, 224, uint32_t, 0xc1059ed8, 0x367cd507, 0x3070dd17, 0xf70e5939, 0xffc00b31, 0x68581511, 0x64f98fa7, 0xbefa4fa4>;
using Hash_sha2_256 = Hash<0xb, 256, uint32_t, 0x6a09e667, 0xbb67ae85, 0x3c6ef372, 0xa54ff53a, 0x510e527f, 0x9b05688c, 0x1f83d9ab, 0x5be0cd19>;
using Hash_sha2_384 = Hash<0xc, 384, uint64_t, 0xcbbb9d5dc1059ed8, 0x629a292a367cd507, 0x9159015a3070dd17, 0x152fecd8f70e5939, 0x67332667ffc00b31, 0x8eb44a8768581511, 0xdb0c2e0d64f98fa7, 0x47b5481dbefa4fa4>;
using Hash_sha2_512 = Hash<0xd, 512, uint64_t, 0x6a09e667f3bcc908, 0xbb67ae8584caa73b, 0x3c6ef372fe94f82b, 0xa54ff53a5f1d36f1, 0x510e527fade682d1, 0x9b05688c2b3e6c1f, 0x1f83d9abfb41bd6b, 0x5be0cd19137e2179>;
