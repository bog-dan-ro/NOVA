...
class Sha
{
    ...
    protected:
        ...
        template<typename T> void preprocess (T *h, uint8_t const *m, size_t s, bool more, void (*hash)(T *, uint8_t const *))
        {
            ...
            *reinterpret_cast<Unaligned_be<L> *>(b + res_size) = 8 * len;
...
template<unsigned F, unsigned D, unsigned H, typename T, T... I> class Hash : private Sha
{
    private:
        T h[H] { I... };

        static constexpr void (*hash[])(T *, uint8_t const *) { sha1, sha2 };
    ...
    public:
        ...
        {
            for (unsigned i { 0 }; i < digest / sizeof (T); i++)
                reinterpret_cast<Unaligned_be<T> *>(p)[i] = h[i];
        }

        void update (uint8_t const *m, size_t s, bool more = false) { preprocess (h, m, s, more, hash[F]); }
};

// Section 5.3
using Hash_sha1_160 = Hash<0, 20, 5, uint32_t, 0x67452301, 0xefcdab89, 0x98badcfe, 0x10325476, 0xc3d2e1f0>;
using Hash_sha2_224 = Hash<1, 28, 8, uint32_t, 0xc1059ed8, 0x367cd507, 0x3070dd17, 0xf70e5939, 0xffc00b31, 0x68581511, 0x64f98fa7, 0xbefa4fa4>;
using Hash_sha2_256 = Hash<1, 32, 8, uint32_t, 0x6a09e667, 0xbb67ae85, 0x3c6ef372, 0xa54ff53a, 0x510e527f, 0x9b05688c, 0x1f83d9ab, 0x5be0cd19>;
using Hash_sha2_384 = Hash<1, 48, 8, uint64_t, 0xcbbb9d5dc1059ed8, 0x629a292a367cd507, 0x9159015a3070dd17, 0x152fecd8f70e5939, 0x67332667ffc00b31, 0x8eb44a8768581511, 0xdb0c2e0d64f98fa7, 0x47b5481dbefa4fa4>;
using Hash_sha2_512 = Hash<1, 64, 8, uint64_t, 0x6a09e667f3bcc908, 0xbb67ae8584caa73b, 0x3c6ef372fe94f82b, 0xa54ff53a5f1d36f1, 0x510e527fade682d1, 0x9b05688c2b3e6c1f, 0x1f83d9abfb41bd6b, 0x5be0cd19137e2179>;
