...
class Capability final
{
    ...
    public:
        ...
        enum class Perm_sm : unsigned
        {
            ...
            DEFINED_INT = ASSIGN | CTRL_DN | CTRL_UP,
        };

        // DC Object Capability Permissions
        enum class Perm_dc : unsigned
        {
            ASSIGN_DEV  = BIT (0),
            ASSIGN_INT  = BIT (1),
            DEFINED     = ASSIGN_INT | ASSIGN_DEV,
        };
        ...
        ALWAYS_INLINE
        inline auto validate (Perm_dc p) const { return validate (Kobject::Type::DC, std::to_underlying (p)); }
