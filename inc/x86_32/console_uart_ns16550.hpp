...
class Console_uart_ns16550 : protected Console_uart
{
    private:
        ...
        {
            if (mmap) [[likely]]
                return *reinterpret_cast<uint8_t volatile *>(mmap + (std::to_underlying (r) << regs.shl));
            else
                return Io::in<uint8_t>(static_cast<port_t>(regs.pio + std::to_underlying (r)));
        }

        void write (Reg8 r, uint8_t v) const
        {
            if (mmap) [[likely]]
                *reinterpret_cast<uint8_t volatile *>(mmap + (std::to_underlying (r) << regs.shl)) = v;
            else
                Io::out<uint8_t>(static_cast<port_t>(regs.pio + std::to_underlying (r)), v);
        }
