...
{
    private:
        static Console_uart_ns16550 uart[];
        ...
        auto read  (Reg8 r) const     { return *reinterpret_cast<uint8_t volatile *>(mmap + (std::to_underlying (r) << regs.shl)); }
        void write (Reg8 r, uint8_t v) const { *reinterpret_cast<uint8_t volatile *>(mmap + (std::to_underlying (r) << regs.shl)) = v; }

        void tx (uint8_t c) const override final { write (Reg8::THR, c); }

        [[nodiscard]] bool tx_busy() const override final { return !(read (Reg8::LSR) & BIT (6)); }
        [[nodiscard]] bool tx_full() const override final { return !(read (Reg8::LSR) & BIT (5)); }

        [[nodiscard]] bool match_dbgp (Debug::Type t, Debug::Subtype s) const override final
        {
    ...
    protected:
        explicit Console_uart_ns16550 (Hpt::OAddr m, unsigned c) : Console_uart { c } { setup (Regs { .mem = m, .shl = 0 }); }
};
